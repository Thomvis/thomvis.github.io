...
That's when I had the idea to hook up GPT-4 to my phone. Granted, my phone is not _the_ world, but it sure is a _window_ to my world. A significant one.

## Sense & Control
I built a system on top of the iOS [accessibility](https://developer.apple.com/documentation/accessibility) APIs through which GPT-4 can "see" and interact with what's on the screen. It uses the same textual description of the UI that is used by VoiceOver.

The system runs as a [User Interface Test](https://developer.apple.com/documentation/xctest/user_interface_tests), which is the only way using public API to programmatically control an iOS device. The downside of this approach is that it requires a MacBook with Xcode on it and that it displays an overlay on the device indicating that a test is running. This can probably be aleviated by using private API or a jailbroken device, both of which were outside the scope of my experiments.

For example, this is how GPT-4 perceives the empty list view of the stock Reminders app:
...
To let GPT-4 interact with the phone, I tell it what it can do and how the response should be formatted:
...
My system parses GPT-4's response and executes the corresponding action using methods on UIElement, such as [tap](https://developer.apple.com/documentation/xctest/xcuielement/1618666-tap).

## Prompts
I'm using OpenAI's [chat completion](https://platform.openai.com/docs/api-reference/chat/create) API with model `gpt-4` and temperature `0` (to make the responses deterministic). The first request contains two messages: a "system" to set its general behavior and a "user" message with the description of the initial screen and the aforementioned instructions on how to reply.

The system message reads:
...
On subsequent requests, I replace previous "user" messages with "(UI description omitted)" to save on OpenAI usage fees. (The UI descriptions take up a lot of tokens.) I do include GPT-4's responses as "assistant" messages. My thinking is that the observations in its responses would give enough context so that the previous UI descriptions could be omitted. The "observation" and "thought" fields are based on what I've seen [LangChain](https://python.langchain.com/en/latest/modules/agents/getting_started.html) do.

## Tasks
The remainder of this post is a selection of tasks I had GPT-4 perform. I use each of them to point out something I found interesting. I'll show that this approach comes with a bunch of caveats. It's slow and inefficient. But at the same time, the fact that this works at all is oh so fascinating.
...
### Safari
By giving it access to my phone I'm giving it access to the whole internet, through Safari.
...
### Using a typical third-party app
